  ...
  void ec_update(uint8_t * domain_address)
  {
    ...
    if (pdo_type == TPDO) {
        ...
        !override_command)
      {
        ...
        std::cout << (factor * command_interface_ptr_->at(interface_index) + offset)<<"default"<< default_value<<"interface_index"<<interface_index<<"index"<<index<< std::endl;
        if(index==0x607a){
          int p_val = factor * command_interface_ptr_->at(interface_index) + offset;
          int c_val = default_value;
          int max_gap = 10000;
          int val_send = p_val;

          if(p_val>c_val+max_gap){
            val_send = c_val + max_gap;
          }  
          else if(p_val<c_val-max_gap){
            val_send = c_val - max_gap;
          }          
          ec_write(domain_address, val_send);
        }
        else{
